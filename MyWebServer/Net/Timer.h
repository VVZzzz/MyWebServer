 ...
 * 
 * 这样做也有缺点:因为只有定时器节点真正删除的时候,才会处理handleClose
 * 即断开超时的连接 handleClose(),这样的话可能造成定时器不是太准确,因为只有在
 * loop循环中,poll返回了(有事件就绪或者超过10s等待时间)才能处理定时器超时事件.
 * 这里没有用信号进行打断,多线程程序中最好不用信号
 *
 */
template <typename T>
